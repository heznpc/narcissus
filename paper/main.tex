...
During the collaborative writing process, the AI assistant suggested citations to support specific claims. A post-hoc verification audit identified four cases in which the AI provided citations to real, existing papers but fabricated the author attribution (Table~\ref{tab:hallucinated}).

\begin{table}[htbp]
\centering
\caption{AI-hallucinated citation metadata. All papers exist; author attribution was fabricated.}
...
The 9/11 error rate in a single paper's reference list suggests that the problem is not occasional but systematic. If this rate generalizes across AI-assisted academic writing, it implies a substantial fraction of AI-suggested citations in the broader literature may carry corrupted metadata---a form of silent scholarly pollution that is invisible to readers who do not independently verify every reference.
...
Two cases illustrate the pattern. First, the author's own experience: during an extended AI-assisted application development session, the AI reported the project as complete. A subsequent review in a separate, context-free session revealed that the application was approximately 30\% functional---critical features were missing, error handling was absent, and several components existed only as stubs that the original session had described as implemented. The completion report was not a lie but a context-accumulated optimism: each incremental step had been reported as successful, and the cumulative narrative of success produced a summary assessment disconnected from the artifact's actual state.

Second, a public case reported in March 2026: an immunologist and AI power user tested two AI coding assistants on the same task (creating a pixel RPG game for iPhone). One system consumed 7\% of its allocated resources and produced a fully playable game. The other consumed over 90\% of its allocation while failing to produce a minimally playable result---yet its session-level reporting consistently indicated progress and near-completion. The user cancelled his subscription, noting the gap between the AI's claims and its deliverables.
...
The Narcissus Loop framework and the empirical evidence presented above generate the following testable hypotheses. Each hypothesis specifies the predicted direction and is designed to be empirically falsifiable through the proposed studies in Section~\ref{sec:study-design}.
...
\paragraph{H3 (Awareness Insufficiency).} Researcher awareness of the Narcissus effect does not significantly reduce its magnitude: researchers who have been informed about collaborative entrenchment prior to an AI-assisted writing task show citation directionality ratios and counter-argument densities that are not significantly different from those of uninformed researchers under the same conditions.
...
\begin{itemize}
    \item \textbf{Citation directionality:} Each citation added or removed during revision will be classified as thesis-confirming, thesis-challenging, or neutral by two independent raters. The ratio of confirming to challenging citations in AI-assisted drafts versus post-review corrections is the primary measure of directional bias.
...
This yields four cells: (1)~AI-assisted with Fresh review, (2)~AI-assisted without Fresh review, (3)~Solo with Fresh review, and (4)~Solo without Fresh review.

\textbf{Participants.} Graduate students or early-career researchers ($N \geq 60$, minimum 15 per cell) recruited from multiple disciplines to ensure variability in domain knowledge and research style.
...
\begin{itemize}
    ...
    \item \textbf{Blind reviewer ratings:} Independent reviewers (blinded to condition) rate each paper on argumentative balance, evidentiary rigor, and overall scholarly quality using a standardized rubric.
    \item \textbf{Metadata accuracy:} For the AI-assisted conditions, the rate and directionality of citation metadata errors.
\end{itemize}

\textbf{Predictions.} If collaborative entrenchment is an emergent property of the human--AI interaction loop (rather than a simple sum of confirmation bias and sycophancy), the following pattern should emerge:
\begin{itemize}
    \item Cell~2 (AI-assisted, no Fresh review) will show the highest citation directionality ratio and lowest counter-argument density---higher than Cell~4 (Solo, no review), which reflects confirmation bias alone without AI amplification.
    \item The interaction effect (AI $\times$ no review) will be significant and super-additive: the AI-assisted bias will exceed the sum of the Solo bias main effect and the AI sycophancy main effect.
    \item Cell~1 (AI-assisted, Fresh review) will show citation directionality and counter-argument density comparable to or better than Cell~3 (Solo, Fresh review), demonstrating that the Fresh-review intervention effectively breaks the loop.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Model specificity.} All observations were made with a single model family (Claude). Other LLMs may exhibit different sycophancy profiles, different hallucination patterns, and different context-sensitivity characteristics. The generalizability of the Narcissus Loop across AI systems is an open empirical question that future work should address.
\end{itemize}
