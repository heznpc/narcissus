...
Third, \emph{4.7 is somewhat more variable than 4.6} within version: mean Fleiss' $\kappa$ drops from 0.276 (4.6, fair) to 0.203 (4.7, slight/fair border) across the five papers. This is not direct evidence that 4.7 is less reliable as a reviewer; it may reflect higher generation diversity that enriches the union of stable findings (4.7 introduces 1--4 unique stable sections per paper relative to 4.6). The data discriminate weakly between this and a true reliability decrease; further runs ($n \geq 10$ per cell) would be needed to separate the hypotheses.
...
\begin{itemize}
    ...
    \item \textbf{Model specificity.} All observations in the natural experiment were made with a single model family (Claude) and a single version (Opus 4.6). Claude Opus 4.7, released after the audit was complete, may exhibit different sycophancy and hallucination profiles. A preliminary 50-cell multi-run replication (5 papers $\times$ 2 model versions $\times$ 5 runs, executed 2026-05-21) found that mean cross-version pairwise Jaccard between (4.6, 4.7) review pairs (0.27) is only modestly lower than within-4.6 pairwise Jaccard (0.35), suggesting the diagnosis is largely model-architecture-stable within the Claude family. However, intra-version Fleiss' $\kappa$ is fair-to-slight (0.27 for 4.6, 0.20 for 4.7), and a substantial fraction of any single audit's findings is sample-dependent. Cross-vendor generalization to GPT, Gemini, and open-weights families remains open and is the focus of a separate planned study (TODO~\#2). The generalizability of the Narcissus Loop across model vendors is an open empirical question that future work should address.
